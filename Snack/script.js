...
function buildShareLink(finalScore) {
  const base = window.location.href.split('#')[0];
  const hash = `s=${finalScore}&m=${encodeURIComponent(mode)}&d=${encodeURIComponent(difficulty)}`;
  return `${base}#${hash}`;
}
...
if (copyShareBtn && shareLinkInput) {
  copyShareBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      setTimeout(() => (copyShareBtn.textContent = '复制成绩链接'), 1500);
    } catch (e) {
...
// when game ends, populate share link
const originalEndGame = endGame;
endGame = function() {
  originalEndGame();
  if (shareLinkInput) {
    shareLinkInput.value = buildShareLink(score);
  }
};

// if page loaded with a shared score in hash, show a small message
(function checkSharedHash(){
  ...
  if (s) {
    overlay.classList.remove('hidden');
    statusText.textContent = `分享：得分 ${s}（模式:${m||'--'} 难度:${d||'--'}）`;
  }
})();
